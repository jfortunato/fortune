...
Feature: Resource
    ...
    Scenario Outline: Getting Access Denied error when owner is required and we are not the resource owner
        Given I am not logged in
        And The "Fortune\Test\Entity\Dog" resource requires owner for access
        When I send a <method> request to "<url>"
        Then The response code should be 403
    Examples:
            | method | url     |
